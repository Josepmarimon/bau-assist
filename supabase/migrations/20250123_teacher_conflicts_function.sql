-- Create a function to check teacher schedule conflicts considering semesters
-- This ensures teachers are not double-booked within the same semester
...
COMMENT ON FUNCTION check_teacher_schedule_conflicts IS 'Checks if a teacher has schedule conflicts within a specific semester. Only returns conflicts for the same time slot within the same semester.';

-- Create a more comprehensive function that checks all teachers in a subject group
...
-- Comment the function
COMMENT ON FUNCTION check_subject_group_teacher_conflicts IS 'Checks if any teacher assigned to a subject group has schedule conflicts within the specified semester.';