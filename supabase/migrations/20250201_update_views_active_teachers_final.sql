...
DROP FUNCTION IF EXISTS get_teachers_for_group(UUID);
...
SELECT 
    ...
    CASE 
        WHEN t.is_active = true OR t.is_active IS NULL THEN COALESCE(a.teacher_id, tga.teacher_id)
        ELSE NULL
    END as teacher_id,
    -- Include teacher details only if active
    CASE WHEN t.is_active = true OR t.is_active IS NULL THEN t.first_name ELSE NULL END as teacher_first_name,
    CASE WHEN t.is_active = true OR t.is_active IS NULL THEN t.last_name ELSE NULL END as teacher_last_name,
    CASE WHEN t.is_active = true OR t.is_active IS NULL THEN t.email ELSE NULL END as teacher_email,
...
WHERE tga.academic_year = '2025-2026'
  AND (t.is_active = true OR t.is_active IS NULL)  -- Only show active teachers
...
-- 3. Recreate function that returns teachers to filter inactive ones
CREATE FUNCTION get_teachers_for_group(group_id UUID)
RETURNS TABLE (
    teacher_id UUID,
    first_name TEXT,
    last_name TEXT,
...
AS $$
    ...
    AND tga.academic_year = '2025-2026'
    AND (t.is_active = true OR t.is_active IS NULL)  -- Only return active teachers
...
AS $$
    ...
    AND (t.is_active = true OR t.is_active IS NULL)  -- Only include active teachers
